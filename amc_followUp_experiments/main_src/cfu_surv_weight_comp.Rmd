...
culture_tax_table <- read_tsv(culture_tax_fp) %>% 
  ...
  dplyr::select(mouse_id, location, cult_rel_abund, top_tax) 
```
**Data wrangled**
...
mini_cfu_table <- cfu_table %>% 
  dplyr::select(mouse_id, location, colony_count)

proc_raw_cfu_table <- raw_cfu_table %>% 
  dplyr::select(mouse_id.tissue, `colonies_per_g/mL`) %>% 
...
(facet_status_cfu_plot <- surv_cfu_table %>% 
  ...
  geom_jitter(aes(fill = top_tax, shape = vendor), 
              width = 0.1, height = 0.1, size = 4, alpha = 0.8) +
