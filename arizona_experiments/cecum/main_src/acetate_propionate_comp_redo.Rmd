...
stat_on_plot <- function(stat_table,
  ...
  ## create table based on nested list to join w stat table 
  ## this makes it a lot easier to specify different locations for the 
  ## stats on the plot
  stat_loc_table <- enframe(product_list, 
                            name = join_col, 
                            value = 'tmp') %>% 
    unnest_wider(tmp)
  ...
  proc_stat_table <- stat_table %>% 
    ...
    left_join(stat_loc_table, by = join_col) %>% 
    ...
    mutate("{x_col_name}" := as.numeric(.data[[x_col_name]]),
...
histo_metabTox_table %>% 
  select(mouse_id, microbe_product, conc_normalized, log_conc_normalized) %>% 
  filter(microbe_product %in% names(metab_labs)) %>%
  left_join(histoTox_only_table, by = 'mouse_id',
            relationship = "many-to-many") %>% 
  mutate(conc_score = as.numeric(conc_score),
         conc_normalized = as.numeric(conc_normalized)) -> all_metab_table
```
...
metab_tox_table <- all_metab_table %>% 
  ...
  rename(log_conc_norm = log_conc_normalized) %>% 
  mutate(log_conc_score = log10(conc_score))
...
metab_tox_plot <- apppleplots::correlation_plots(input_table = metab_tox_table,
                                                    ...
                                                    title_content = 'Cecum SCFA Comparisons')
...
## stats on plot
tox_stat_loc <- list('Acetic Acid (ug/g)' = c('conc_normalized' = 6.5,
                                               'conc_score' = 1000),
                     'n-Butanoic Acid (ug/g)' = c('conc_normalized' = 20,
                                                  'conc_score' = 10),
                     'Propanoic Acid (ug/g)' = c('conc_normalized' = 13,
                                                 'conc_score' = 10))

metab_tox_list <- stat_on_plot(stat_table = metab_tox_stats,
                               wanted_col_list = colnames(metab_tox_stats),
                               product_list = tox_stat_loc,
                               join_col = 'microbe_product',
...
## stats on plot
histo_stat_loc <- list('Acetic Acid (ug/g)' = c('conc_normalized' = 6.5,
                                                'conc_score' = 4.5),
                       'n-Butanoic Acid (ug/g)' = c('conc_normalized' = 800,
                                                    'conc_score' = 5.5),
                       'Propanoic Acid (ug/g)' = c('conc_normalized' = 5,
                                                   'conc_score' = 5.75))

metab_histo_list <- stat_on_plot(stat_table = metab_histo_stats,
                               wanted_col_list = colnames(metab_histo_stats),
                               product_list = histo_stat_loc,
                               join_col = 'microbe_product',
                               x_col_name = 'conc_normalized',
                               y_col_name = 'conc_score',
...
## just saving the comparisons
save(all_metab_plotsWithlab,
     file = '../../../figures/all_scfa_comp_plot.rdat')
```
