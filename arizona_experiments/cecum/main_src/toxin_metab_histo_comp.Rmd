...
toxin_FP <- '../data/misc/v2_proc_neat_toxin.tsv'
...
metab %>% 
  ...
  mutate(metab_conc_norm = (as.numeric(concentration) + 2),
         log_metab_conc = log10(metab_conc_norm)) -> metab
...
toComb_histo_metab <- histo_metab %>% 
  rename(microbe_product = metabolite,
         ...
         log_conc_normalized = log_metab_conc) 
...
toComb_histo_neatTox <- histo_neat_toxin %>% 
  rename(microbe_product = neat_toxin,
         ...
         log_conc_normalized = log_neat_conc)
