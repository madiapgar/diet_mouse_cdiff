...
baseline_plot <- function(baseline_table,
  ...
  baseline_table %>% 
    ...
    scale_fill_gradient2(low = 'blue', high = 'red', limits = c(-6.5, 6.5)) +
...
title <- ggdraw() +
          draw_label('Log Fold Changes Compared to the Diet Baseline (Combined Dataset)',
                     size = 28,
                     fontface = "bold",
                     hjust = 1)
plot_grid(chow_plot,
...
ft_fb_lfc %>% 
  ...
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.title = element_blank()) +
  ggtitle('Log Fold Changes by Diet Composition\n(Combined Dataset)') -> ftFb_lfc_plot
...
ggsave('diet_baseline_plot.pdf',
       ...
       height = 27, 
       path = '../plots')
ggsave('diet_composition_lfc.pdf',
       plot = ftFb_lfc_plot,
       width = 6.5, 
       height = 9.5, 
       path = '../plots')
