...
bile_file_prep <- function(bile_acid_fp,
  ...
  bile_acid %>% 
    ...
    select(temp_id, tube_label, collection_date, 
           sample_type, contains(unlist(inhibitors))) %>% 
  ...
  bile_acid %>% 
    mutate(temp_id = paste(tube_label, reisdorph_lab_id, sep = ".")) %>% 
    select(temp_id, tube_label, collection_date, 
           sample_type, contains(unlist(promoters))) %>% 
    gather(contains('acid'), key = bile_acid, value = concentration) %>% 
...
kruskal <- stat_files$KruskalTest
dunn <- stat_files$DunnPostHoc
...
kruskal_bile <- stat_files$KruskalTest
dunn_bile <- stat_files$DunnPostHoc
```
