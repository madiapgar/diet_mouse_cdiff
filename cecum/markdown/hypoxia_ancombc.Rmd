...
cecum_lfc %>% 
  gather(-taxon, -fiber, key = component, value = lfc) %>% 
...
cecum_lfc %>% 
    ...
    geom_tile(aes(fill = lfc), color = 'black') +
    scale_fill_gradient2(low = 'blue', high = 'red') +
    geom_text(aes(label = lfc), size = 4) +
    theme_bw(base_size = 16) +
    ...
    theme(axis.title.x = element_blank(),
          ...
          legend.title = element_blank(),
          plot.title = element_text(face = "bold")) +
  ggtitle('Family Level Hypoxia Significant\nLog Fold Changes') -> cecum_lfc_family_plot

cecum_lfc_family_plot
```
...
**Saving my Outputs**
```{r}
ggsave('../plots/hypoxia_cecum_famLevel.pdf',
       plot = cecum_lfc_family_plot,
       width = 10,
       height = 7)
