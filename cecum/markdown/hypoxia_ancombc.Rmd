...
Encode the diets by high fiber, low fiber, and purified diet. Put everything together. Consider interactions between diets. Stratify by location and run the model by dietary fiber content!!
...
# **ANCOMBC Between Dietary Components ONLY**
If I run this stratified by location it gives me the exact same values for each location because we sampled the same mice three times (for the measurements in three different locations). 

**ANCOMBC**
```{r, warning=FALSE}
diet_res <- run_ancombc(otu_table_FP,
                        tax_FP,
                        hypox_metadata_long,
                        'Family',
...
diet_ancom <- diet_res$ANCOMBCResults
```
...
diet_ancom %>% 
  ...
  select(taxon, contains('corr')) -> diet_comp_lfc

colnames(diet_comp_lfc) <- c('taxon',
                             'high_fat',
                             'high_fiber',
                             'purified_diet')
...
diet_comp_lfc %>% 
  ...
  mutate(lfc = as.numeric(lfc),
         lfc = signif(lfc, digits = 5),
         lfc = ifelse(is.na(lfc), 0, lfc)) -> diet_comp_lfc
```

**Plot**
```{r, fig.width=10, fig.height=7}
diet_comp_plot <- baseline_plot(diet_comp_lfc,
                               NULL,
                               NULL,
                               diet_comp_x_labs,
                               'Log Fold Change of Hypoxia by\nDietary Component (Family Level)')
