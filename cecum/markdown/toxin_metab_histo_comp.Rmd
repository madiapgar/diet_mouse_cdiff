...
Trying something new - over laying the p-value significance on the plot.
```{r, fig.height=4, fig.width=9}
neatTox_stat_table %>% 
  filter(tissue == 'cecum',
         signif != 'ns') %>% 
  select(diet, tox_metab, signif, p_value) %>% 
...
neat_histo_cecum_plot +
  geom_label(data = procNeat_stat_table, 
            label = procNeat_stat_table$p_value,
            color = 'black',
            fontface = 'bold',
            size = 3) -> neat_cecumHisto_stats_plot
...
metab_stat_table %>% 
  filter(tissue == 'cecum',
         signif != 'ns') %>% 
  select(diet, tox_metab, signif) %>% 
  mutate(metab_conc_norm = paste(1000),
         metab_conc_norm = as.numeric(metab_conc_norm),
         score = paste(5.5),
         score = as.numeric(score)) -> metab_stat_table2

names(metab_stat_table2)[names(metab_stat_table2) == 'tox_metab'] <- 'metabolite'

histo_metabCecum_plot +
  geom_text(data = metab_stat_table2, 
            label = metab_stat_table2$signif,
            color = 'red',
            size = 6) -> metab_cecumHisto_stats_plot
