...
## **ANCOMBC Pairwise results for ASV by Diet - These were Run in R**
...
**Data Wrangling Pairwise LFCs**
```{r}
test_day_filt %>% 
  filter(`diff_dietHF/LF_dietHF/HF` == TRUE | `diff_dietLF/HF_dietHF/HF` == TRUE | `diff_dietLF/LF_dietHF/HF` == TRUE | `diff_dietLF/HF_dietHF/LF` == TRUE | `diff_dietLF/LF_dietHF/LF` == TRUE | `diff_dietLF/LF_dietLF/HF` == TRUE) %>% 
  mutate(lfc_hftLfb_hftHfb_corr = (`lfc_dietHF/LF_dietHF/HF` * `diff_dietHF/LF_dietHF/HF`),
         lfc_lftHfb_hftHfb_corr = (`lfc_dietLF/HF_dietHF/HF` * `diff_dietLF/HF_dietHF/HF`),
         lfc_lftLfb_hftHfb_corr = (`lfc_dietLF/LF_dietHF/HF` * `diff_dietLF/LF_dietHF/HF`),
         lfc_lftHfb_hftLfb_corr = (`lfc_dietLF/HF_dietHF/LF` * `diff_dietLF/HF_dietHF/LF`),
         lfc_lftLfb_hftLfb_corr = (`lfc_dietLF/LF_dietHF/LF` * `diff_dietLF/LF_dietHF/LF`),
         lfc_lftLfb_lftHfb_corr = (`lfc_dietLF/LF_dietLF/HF` * `diff_dietLF/LF_dietLF/HF`)) %>% 
  select(taxon, day_post_inf, contains('corr')) -> test_day_lfc

colnames(test_day_lfc) <- c('taxon',
                             'day_post_inf',
                              'HF/LF : HF/HF',
                              'LF/HF : HF/HF',
                              'LF/LF : HF/HF',
...
test_day_lfc %>%
  gather(!c(taxon, day_post_inf), key = diet_pair, value = lfc) %>% 
  mutate(lfc = round(lfc, 2)) -> test_day_lfc
```

**Pairwise LFC Plot**
```{r, fig.width=14, fig.height=6}
test_day_lfc %>% 
  ggplot(aes(x = diet_pair, y = taxon)) +
  geom_tile(aes(fill = lfc), color = 'black') +
  scale_fill_gradient2(low = 'blue', high = 'red') +
  geom_text(aes(label = lfc), size = 3) +
  theme_bw() +
  scale_x_discrete(labels = c('HF/LF:\nHF/HF',
                              'LF/HF:\nHF/HF',
                              'LF/LF:\nHF/HF',
                              'LF/HF:\nHF/LF',
                              'LF/LF:\nHF/LF',
...
**Trying out changing baseline**
Going to rbind these together for a plot.
```{r}
## HF/HF baseline
test_baseline <- ancom_day_for_loop(otu_table_FP,
                                     tax_FP,
                                     metadata,
                                     'Family',
                                     'diet',
                                    ...
                                    'HF/HF',
                                     day_strat)
test_baseline %>% 
  mutate(baseline_diet = "HF/HF") -> test_baseline
...
all_diet_lfc %>% 
  ...
  #                             'LFt/\nHFb',
  #                             'LFt/\nLFb')) +
  facet_grid(diet~baseline_diet) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.title = element_blank()) +
  ggtitle('Log Fold Changes by Diet Baseline')
```
