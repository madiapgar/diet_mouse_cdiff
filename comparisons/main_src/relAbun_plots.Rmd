...
## plot function for all other plots
...
## plot function for manuscript figure plot
special_abun_plots <- function(input_table,
                               x_axis,
                               y_axis,
                               x_group_by,
                               line_group_by,
  ...
  plot <- input_table %>%
      ...
      scale_y_continuous(trans = 'log10') +
      geom_boxplot(aes(group = .data[[x_group_by]]), outlier.shape = NA) +
      geom_line(aes(group = .data[[line_group_by]], color = .data[[line_color_by]]), 
                alpha = line_alpha) +
      geom_jitter(aes(fill = .data[[fill_by]], shape = .data[[shape_by]]), 
                  width = 0.1, height = 0.1, alpha = 0.7, size = 4) +
      scale_fill_viridis(option = viridis_palette,
                         discrete = TRUE,
                         name = legend_name,
                         labels = legend_labels) +
      ...
      facet_grid(rows = vars(.data[[facet_rows]]),
                 cols = vars(.data[[facet_cols]]),
                 labeller = labeller(.cols = facet_col_labs)) +
      theme(strip.text.y.right = element_text(angle = 0),
            legend.position = "top") +
      ggtitle(title) +
      ylab(y_name) +
...
## diet
special_exp_labs <- c('1' = 'Previous AMC (published)\n(AMC-P)',
                      '2' = 'University of Arizona\n(UA)',
                      '3' = 'AMC Follow-Up\n(AMC-FU)')

(miniDiet_genusAbun_plot <- special_abun_plots(input_table = mini_genusAbun_table,
                                                x_axis = 'day_fact',
                                                y_axis = 'rel_abund',
                                                x_group_by = 'day_fact',
                                                ...
                                                legend_labels = c('Chow',
                                                                  ...
                                                                  'LFt / HFb',
                                                                  'LFt / LFb'),
                                                shape_legend_name = 'Vendor',
                                                shape_legend_labels = c('Charles\nRiver',
                                                                        'Taconic'),
                                                facet_rows = 'Genus',
                                                facet_cols = 'exp_order',
                                                facet_col_labs = special_exp_labs,
                                                title = 'Changes in Genera from Baseline to Day 3',
...
cdiff_genusAbun_table <- genus_abun_table %>% 
  filter(Genus == 'Clostridioides',
         day_post_inf == 3)

diet_x_labs <- c('Chow' = 'Chow',
                 'HF/HF' = 'HFt/\nHFb',
                 'HF/LF' = 'HFt/\nLFb',
                 'LF/HF' = 'LFt/\nHFb',
                 'LF/LF' = 'LFt/\nLFb')
...
(cdiff_genusAbun_plot <- apppleplots::facet_twice_plots(input_table = cdiff_genusAbun_table,
                               x_value = 'diet',
                               y_value = 'rel_abund',
                               ...
                               facet_cols = 'exp_order',
                               row_labs = NULL,
                               col_labs = special_exp_labs,
                               x_name = 'Diet',
                               y_name = 'Relative Abundance (log10)',
                               title_content = 'Differences in Clostridioides Abundance') +
  theme_bw(base_size = 20) +
  facet_grid(cols = vars(exp_order),
             scales = 'free_x',
             labeller = labeller(exp_order = special_exp_labs)) +
...
ggsave('../baseline_day3/plots/diet_genusAbun_plot.pdf',
       ...
       width = 15, 
       height = 13)
ggsave('../baseline_day3/plots/cdiff_genusAbun_plot.pdf',
       plot = cdiff_genusAbun_plot,
       width = 12, 
       height = 6)
...
save(cdiff_genusAbun_plot,
     file = '../../figures/cdiffComparison_genusAbun_plot.rdat')
```
