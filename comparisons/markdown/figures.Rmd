...
**Trying to read in the fluorescence images as pngs**
```{r}
hypox_dam_fp <- '../../figures/hypox_damage.png'
hypox_NoDam_fp <- '../../figures/hypox_NoDamage.png'

## damaged low fat/low fiber image 
hypox_dam <- readImage(img_path = hypox_dam_fp,
                       resize = NULL,
                       rotate = NULL)
test_rc <- recolorize2(hypox_dam)

test_vector <- recolorizeVector(test_rc,
                                size_filter = 0.05,
                                smoothness = 5,
                                plotting = TRUE)

hypox_dam_plot <- plotImageArray(test_rc$original_img) 

## not damaged low fat/high fiber image
hypox_no_dam <- readImage(img_path = hypox_NoDam_fp,
                         resize = NULL,
                         rotate = NULL)

hypox_no_dam_plot <- plotImageArray(hypox_no_dam) 
...
```{r, warning=FALSE, fig.height=13.5, fig.width=23}
