...
**Linear Modeling for Wanted Family Abundances**
```{r}
## for overall plot 
long_abun_filt %>%
  na.omit() %>% 
...
long_abun_filt %>% 
  ...
  mutate(p.adj = p.adjust(p.value,
                          method = "BH"),
         test_id = paste(Family, day_post_inf, sep = "_")) %>% 
  filter(p.adj <= 0.05) -> long_kruskal

long_abun_filt %>% 
  na.omit() %>% 
  group_by(Family, day_post_inf) %>%
  mutate(test_id = paste(Family, day_post_inf, sep = "_")) %>% 
  filter(test_id %in% long_kruskal$test_id) %>% 
...
long_abun_filt %>% 
  ...
  summarise(mean_rel_abund = mean(rel_abund)) -> mean_family_abun_long

long_dunn %>% 
  merge(mean_family_abun_long, 
        by.x = c('group1',
                 'day_post_inf',
                 'Family'),
        by.y = c('diet',
  ...
  mutate(diff_means = (group1_rel_abun - group2_rel_abun),
         stat_diff_means = if_else(p.adj > 0.05, 0, diff_means)) -> new_long_dunn
```
...
```{r, fig.height=7.5, fig.width=14.5}
