...
big_histo %>% 
  group_by(tissue) %>% 
  do(tidy(kruskal.test(score ~ day_post_inf,
                       data = .))) -> all_day_kruskal

big_histo %>% 
  group_by(tissue) %>% 
  dunn_test(score ~ day_post_inf,
            p.adjust.method = 'BH',
            data =.) %>% 
...
big_histo %>% 
  ...
  ggplot(aes(x = day_post_inf, y = score)) +
  geom_violin(aes(group = day_post_inf),  draw_quantiles = c(0.5)) +
  geom_jitter(aes(fill = diet), alpha = 0.4, width = 0.1, height = 0, pch = 21) +
  scale_fill_brewer(palette = 'Dark2', name = 'Diet') +
  scale_x_discrete(breaks = c(-15, -8, -3, 0, 3)) +
  facet_wrap(~tissue, labeller = labeller(tissue = tissue_labs),
             scales = "free_y") +
  ...
  ylab('Histopathology Score') +
  ggtitle("Histopathology Score by Days Relative to Infection") -> all_day_plot

all_day_plot
```

**Overall Mouse Timepoint Linear Modeling**\
running a linear model on the histopathology of all mice, regardless of day of death and cecal removal.

```{r}
# big_histo %>% 
#   group_by(tissue) %>% 
#   do(glance(lm(score ~ purified_diet + high_fat * high_fiber,
#              data =.))) %>% 
#   na.omit() %>% 
#   arrange(p.value) %>% 
