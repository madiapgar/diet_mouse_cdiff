...
comp_plot <- function(metab_table,
  ...
  metab_table %>% 
    select(mouse_id, metabolite, log_conc) %>% 
  ...
  compiled_table %>% 
    ...
    na.omit() %>% 
    ggplot(aes(x = .data[[wanted_x]], y = log_conc)) +
...
r2_value <- function(biom_table,
  ...
  biom_table %>% 
    ...
    do(glance(lm(log_conc ~ .data[[wanted_conc_value]],
                 data = .))) -> lm_glance
  ...
  biom_table %>% 
    filter(diet == wanted_diet,
           metabolite == wanted_metab,
           .data[[tox_col]] == wanted_tox) %>% 
    do(tidy(lm(log_conc ~ .data[[wanted_conc_value]],
                 data = .))) %>% 
