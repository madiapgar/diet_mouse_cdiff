...
put mouse id in this model under rand_formula OR stratify by day and run ancombc for every single day !! 
can parse in filtered tables to phyloseq (not the qiime2 one) so you don't have to create new files for every single day

**Trying out my new for loop function**
```{r}
...
it's literally giving me the same values for everything?? why?? 
each day is exactly the same as the original plot that I have done just by diet without taking day into account. 
...
**Running ANCOMBC**
```{r}
diet_ancom <- run_ancombc(otu_table_FP,
                           tax_FP,
                           metadata,
                           'Family',
                           'diet',
...
diet_res <- diet_ancom$ANCOMBCResults
```
...
diet_res %>% 
  ...
  mutate(lfc_hftHfb_corr = (`lfc_dietHF/HF` * `diff_dietHF/HF`),
         ...
         lfc_lftHfb_corr = (`lfc_dietLF/HF` * `diff_dietLF/HF`),
         lfc_lftLfb_corr = (`lfc_dietLF/LF` * `diff_dietLF/LF`)) %>% 
  select(taxon, contains('corr')) -> diet_lfc

colnames(diet_lfc) <- diet_col_names

diet_lfc %>% 
  gather(!c(taxon), key = diet, value = lfc) %>% 
...
diet_lfc %>% 
  ...
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.title = element_blank()) +
  ggtitle('Log Fold Changes\nCompared to Mice on the Chow Diet') -> diet_lfc_plot
...
```{r}
metadata %>% 
  filter(day_post_inf == -8) -> test_meta

run_ancombc(otu_table_FP,
            tax_FP,
            test_meta,
            'Family',
            'diet + seq_depth',
            'BH') -> test_res
test_res$ANCOMBCResults
```
...
ft_fb_res <- run_ancombc(otu_table_FP,
                                   ...
                                   metadata,
                                   'Family',
                                   'high_fat + high_fiber + purified_diet + diet',
                                   'BH',
                                   day_strat)
