...
## joining culture relative abundance table
proc_cult_genusAbun <- cult_genusAbun_table %>% 
                          select(mouse_id, Genus, rel_abund, location) %>% 
                          group_by(mouse_id, location) %>% 
                          filter(rel_abund == max(rel_abund)) %>% 
                          mutate(top_tax = ifelse(rel_abund >= 0.7, Genus, "Polymicrobial"),
                                 top_tax = ifelse(is.na(top_tax), 'Unknown', top_tax)) %>% 
...
comb_cfu_genusAbun <- proc_cfu_count %>% 
  ...
  mutate(top_tax = ifelse(is.na(top_tax), 'None', top_tax),
         top_tax = ifelse(top_tax == 'Micrococcus' | top_tax == 'Oceanobacillus' | top_tax == 'Dietzia' |
                            top_tax == 'Gemella' | top_tax == 'Corynebacterium', 'Contamination', top_tax))
```
...
my_palette <- c("#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#999999")
```
...
comb_cfu_genusAbun %>% 
  ...
  facet_nested_wrap(~diet + vendor,
                    scales = "free_x",
                    labeller = labeller(diet = diet_labs,
                                        vendor = vendor_labs),
                    nrow = 2) +
  scale_color_manual(breaks = c('Bacillus',
                                'Enterococcus',
