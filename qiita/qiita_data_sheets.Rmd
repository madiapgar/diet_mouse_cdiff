...
pull_mouseid <- function(input_table_fp,
  ...
  table <- read_tsv(input_table_fp) %>% 
    select(all_of(sampleid_col), mouse_id) %>% 
...
make_qiita_sample_info <- function(input_table,
  ...
  qiita_sampleInfo_table <- input_table %>% 
    rename("sample_name" = sampleid_col,
           ## idk if this is right for the tube id 
           tube_id = BarcodeName,
           "collection_date" = collection_date_col) %>% 
    select(sample_name, tube_id, collection_date) %>% 
    mutate(collection_date = paste(gsub(collection_date, pattern = date_delim, replacement = "-")),
           ...
           ## idk if we have any speciman remaining - need to ask keith
           physical_speciman_remaining = paste(FALSE),
           host_age = paste(6),
           host_age_units = paste("weeks"),
           # host_subject_id = paste("MOUSE ID HERE"),
           host_taxid = paste(10090),
           host_scientific_name = paste("Mus musculus"),
           host_common_name = paste("mouse"),
           ## idk if these mice are juvenile or adult
           host_life_stage = paste("juvenile"),
           sex = paste("female"),
           host_height = paste("not applicable"),
           host_height_units = paste("not applicable"),
           # host_weight = paste("MOUSE STARTING WEIGHT GOES HERE"),
...
make_qiita_prep_info <- function(input_table,
  ...
  qiita_prepInfo_table <- input_table %>% 
      ...
      mutate(experiment_design_description = paste(expDes_description_entry), ## changes 
             ...
             ## are these primers correct??
             pcr_primers = paste('FWD:GTGCCAGCMGCCGCGGTAA; REV:GGACTACHVGGGTWTCTAAT'),
...
```{r}
qiita_sampleInfo_fp <- '../../qiita_templates/QIITA_sample_information.txt'
...
```{r}
## tsvs
fp_list <- c(qiita_sampleInfo_fp,
             s40_fp, s44_fp, s45_fp,
...
named vectors (c()) in r are like dictionaries in python!
```{r}
...
## **arizona stool samples**
NOTE: can't find sampleid 3_0516_9898 or mouse_id 9898 anywhere but its in the sequencing barcodes files !!
```{r}
...
## take out all mock and PCR blank samples
unwanted_az_stool_samples <- az_stool_qiita_dfs %>% 
  filter(sample_name == "PCR Blank" | sample_name == "PCR blank" | sample_name != str_match(sample_name, "Mock")) %>% 
  select(sample_name)

az_stool_qiita_dfs <- az_stool_qiita_dfs %>% 
  filter(!(sample_name %in% unwanted_az_stool_samples$sample_name))

## combine with mouse ids
az_stool_qiita_dfs <- az_stool_qiita_dfs %>% 
  left_join(az_stool_meta, by = 'sample_name') %>% 
...
amc_culture_meta <- read_tsv(amcFU_culture_meta_fp)  %>% 
    select(sampleid, mouse_id, location) %>% 
    rename(sample_name = sampleid,
           host_subject_id = mouse_id) %>% 
...
## take out all mock and PCR blank samples
unwanted_amc_stool_samples <- amc_stool_qiita_dfs %>% 
  filter(sample_name != str_match(sample_name, "PCR_Blank") | sample_name != str_match(sample_name, "EW.Mock")) %>% 
  select(sample_name)

amc_stool_qiita_dfs <- amc_stool_qiita_dfs %>% 
  filter(!(sample_name %in% unwanted_amc_stool_samples$sample_name),
         ## pulling out culture data sampleids 
         !(sample_name %in% amc_culture_meta$sample_name)) %>% 
