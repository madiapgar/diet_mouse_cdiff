...
HISTOPATHOLOGY: 
gather(colon, cecum, key = tissue, value = score) (need these in the same column so I can graph them)
...
faith_tax <- read_tsv(faith_pd_FP)
names(faith_tax)[names(faith_tax) == '#SampleID'] <- 'sampleid'
...
wanted_family <- c('Enterobacteriaceae', 'Lactobacillaceae', 'Lachnospiraceae', 'Enterococcaceae',
                   ...
                   'Marinifilaceae')
faith_tax %>% 
  filter(Family %in% wanted_family) -> faith_tax_filt
...
faith_tax_filt %>%
  filter(!is.na(diet)) %>% 
  ggplot(aes(x = Family, y = faith_pd)) +
  geom_jitter(aes(color = Family), width = 0.1, height = 0, alpha = 0.3) +
  scale_color_brewer(palette = 'Dark2') +
  theme_bw(base_size = 16) +
  facet_grid(day_post_inf~diet, labeller = labeller(diet = diet_labs)) +
  theme(strip.text.y = element_text(angle = 0),
        axis.text.x = element_blank()) +
  ggtitle("Total Sum Scaled Faith's Phylogenic Diversity by Microbe Family") +
  ylab("Faith's PD") +
  labs(x = NULL)

```
Option number two for linking taxonomic information to alpha diversity metrics (for now). 
```{r, warning=FALSE, fig.height=5, fig.width=20}
faith_tax_filt %>%
  filter(!is.na(diet)) %>% 
  ggplot(aes(x = diet, y = Family)) +
  geom_tile(aes(fill = faith_pd), alpha = 0.5) +
