...
```{r}
## also need metadata 
stat_meta <- metadata_fixer('../data/misc/merged_metadata1.tsv')
seq_depths <- read_tsv('../data/misc/tss_seq_depth.tsv')
...
uw_dist_fp <- '../data/qiime/tss_core_metrics3/unweighted_dist/data/'
...
high_fat_diets <- c('HF/HF', 'HF/LF')
low_fat_diets <- c('LF/HF', 'LF/LF', 'Chow')
...
filt_stat_meta %>% 
  ...
  left_join(seq_depths) -> filt_stat_meta
...
## I had to unzip the qza file because it was telling me that it had duplicate names?
## weighted 
w_dist_fp <- '../data/qiime/tss_core_metrics3/weighted_dist/data/'
